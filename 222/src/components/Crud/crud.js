...
function CRUD(options) {
  ...
  const methods = {
    ...
    invalidSuccessNotify() {
      crud.notify(crud.msg.invalid, CRUD.NOTIFICATION_TYPE.SUCCESS)
    },
    // 搜索
    toQuery() {
      crud.page.page = 1
     ...
     */
    toInvalid(data) {
      crud.getDataStatus(crud.getDataId(data)).invalid = CRUD.STATUS.PREPARED
    },
    /**
     * 取消禁用
     * @param {*} data 数据项
     */
    cancelInvalid(data) {
      if (!callVmHook(crud, CRUD.HOOK.beforeInvalidCancel, data)) {
        return
      }
      crud.getDataStatus(crud.getDataId(data)).invalid = CRUD.STATUS.NORMAL
      callVmHook(crud, CRUD.HOOK.afterInvalidCancel, data)
    },
    /**
     * 取消新增/编辑
     */
    ...
    doInvalid(data) {
      ...
      if (!callVmHook(crud, CRUD.HOOK.beforeInvalid, data)) {
        return
      }
      if (!invalidAll) {
        dataStatus.invalid = CRUD.STATUS.PROCESSING
      }
      return crud.crudMethod
        .invalid(ids)
        .then(() => {
          if (invalidAll) {
            crud.invalidAllLoading = false
          } else dataStatus.invalid = CRUD.STATUS.PREPARED
          crud.invalidSuccessNotify()
          callVmHook(crud, CRUD.HOOK.afterInvalid, data)
          // crud.refresh()
        })
        .catch(() => {
          if (invalidAll) {
  ...
  Object.assign(crud, {
    ...
    registerVM(type, vm, index = -1) {
      // console.log(type, vm, (index = -1), 99999)
      const vmObj = {
        type,
        vm: vm
      }
...
CRUD.HOOK = {
  ...
  /** 禁用 - 之前 */
  beforeInvalid: 'beforeCrudInvalid',
  /** 禁用 - 之后 */
  afterInvalid: 'afterCrudInvalid',
  /** 禁用取消 - 之前 */
  beforeInvalidCancel: 'beforeCrudInvalidCancel',
  /** 禁用取消 - 之后 */
  afterInvalidCancel: 'afterCrudInvalidCancel',
  /** 删除 - 之前 */
